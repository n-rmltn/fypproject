...
<!-- Main Section-->
<section class="mt-0 overflow-hidden  vh-100 d-flex justify-content-center align-items-center p-4 rounded">
    <!-- Page Content Goes Here -->

    <!-- Login Form-->
    <div class="col col-md-8 col-lg-6 col-xxl-11 rounded">
        <!-- Logo-->
        <a class="navbar-brand fw-bold fs-3 flex-shrink-0 order-0 align-self-center justify-content-center d-flex mx-0 px-0 text-white"
            href="{{ route('welcome') }}">
        ...
        <div class="shadow-xl p-4 p-lg-5 bg-white">
            ...
            <div class="d-flex justify-content-center mb-5">
                <a href="{{ route('welcome') }}"
                    class="btn rounded bg-dark d-inline-flex m-2 justify-content-center text-white"> Home </a>
                <a href="{{ route('orders') }}"
                    class="btn rounded bg-dark d-inline-flex m-2 justify-content-center text-white"> Orders </a>
                <a href="{{ route('settings') }}"
                    class="btn rounded bg-dark d-inline-flex m-2 justify-content-center text-white"> Settings </a>
                <a href="{{ route('password') }}"
                    class="btn rounded bg-dark d-inline-flex m-2 justify-content-center text-white"> Password </a>
                @if (auth()->user()->is_admin === 1)
                    <a href="{{ route('admin') }}"
                        class="btn rounded bg-success d-inline-flex m-2 justify-content-center text-white"> Admin </a>
                @endif
                <a href="{{ route('logout') }}"
                    class="btn rounded bg-danger d-inline-flex m-2 justify-content-center text-white"> Log Out </a>
            </div>
                                                                                ...
                                                                                <div class="d-flex justify-content-start align-items-start">
                                                                                    ...
                                                                                    <div>
                                                                                        <p class="fw-bolder mb-1 d-flex align-items-center lh-1">
                                                                                            {{ $val->Product->product_name_long }}</p>
                                                                                        <span
                                                                                            class="d-block text-muted">{{ $val->Product->product_name_short }}</span>
                                                                                        <span
                                                                                            class="d-block text-muted">{{ $val->Product->brand->product_brand_name }}</span>
                                                                                    </div>
                                                                                </div>
                                                                            </td>
                                                                            <td class="text-muted">
                                                                                @forelse ($val->Booking_item_option as $opt)
                                                                                    <span
                                                                                        class="d-block text-muted">{{ $opt->Product_option_list->product_option_list_name }}</span>
                                                                                @empty
